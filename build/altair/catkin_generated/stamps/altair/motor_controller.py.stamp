...
class MotorController:
    ...
    def cmd_vel_callback(self, cmd_vel):
        x = int((cmd_vel.linear.x +0.5)* (self.speed_range_max - self.speed_range_min))
        z = int(cmd_vel.angular.z * (self.speed_range_max - self.speed_range_min))
        print(x, z)

        lv = max(min((x + z // 2)+self.speed_range_min, self.speed_range_max), self.speed_range_min)
        rv = max(min((x - z // 2)+self.speed_range_min, self.speed_range_max), self.speed_range_min)
