...
class MotorController:
    ...
    def cmd_vel_callback(self, cmd_vel):
        ...
        lv = int((x - 0.5 * z) * (self.speed_range_max - self.speed_range_min) + self.speed_range_min)
        rv = int((x + 0.5 * z) * (self.speed_range_max - self.speed_range_min) + self.speed_range_min)

        lv = max(min(lv, self.speed_range_max), self.speed_range_min)
        rv = max(min(rv, self.speed_range_max), self.speed_range_min)
